...
spec and asserts its SHA-256 (seed $7$: \texttt{7030fa5d}, seed $11$:
\texttt{f6a648eb}, seed $13$: \texttt{c462c051}); the check is deterministic and needs
no GPU or API key.
...
\paragraph{Run records.} Each run writes a manifest recording the model, the dataset
path and hash, the decoding, the token cap, and the code commit, and one row per
instance carrying the parsed answer, correctness, and token counts; debate additionally
stores every turn of its transcript. Resuming a run against a different model or dataset
is refused. Because the model is served over an API, a re-run is checked at the level of
parsed answers (\texttt{--verify-sample}) rather than byte-identical text.

\paragraph{Statistics.} Wilson intervals, McNemar (continuity-corrected, with the exact
binomial below $25$ discordant pairs), Cochran's $Q$, and the $2{\times}2$ $\chi^2$ are
implemented in the repository on the Python standard library, so no external statistics
package version can move the reported numbers.
