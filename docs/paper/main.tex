...
\title{Does Debate Improve Encoding-Fragile LLM Graph Reasoning?}
...
\label{sec:results}
Per-cell accuracies and Critic diagnostics for all nine cells are in
\ref{sec:percell}.
...
worst - the three largest per-cell gains are the three lowest baseline cells
(Table~\ref{tab:percell-acc}) - so
debate works by lifting the floor, which is what narrows the spread.

\subsection{The gain is the reasoning, not the debate}
\label{sec:decomp}
...
from $0.922$ to $0.803$ (Table~\ref{tab:percell-acc}). That task has a single atomic claim: there is nothing to critique, so the
...
$+0.120$, the debate loop is close to neutral ($-0.003$), and the compute-matched vote
...
(Table~\ref{tab:percell-acc}), and more accurate on three. The full
ordering across the arm is baseline $0.793 <$ debate $0.843 <$ turn~1 $0.864 <$ vote
$0.875$, so debate is the worst of the three conditions and is
...
\label{sec:mechanism}
The debate's failure is not that the Critic cannot tell right from wrong: a
\textsc{revise} tracks a wrong answer in every cell (Table~\ref{tab:percell-critic}).
Split the
...
the single-turn reasoning produces most of the encoding variance reduction (Figure~\ref{fig:fragility}). What reduces a model's
...
wrong answers far more often than on correct ones (Table~\ref{tab:percell-critic}). Yet the debate still lost accuracy,
for a reason that is general. When a model is already mostly correct - as
Llama-3.3-70B is here - revision puts more correct
answers at risk than it can rescue wrong ones, so it does more harm than good.

\paragraph{Reasoning does the work - neither structure earns its compute.}
...
out of a correct answer, whereas a Critic can. The vote is only the less
...
cost, reasoning on the first turn does the work and neither extra structure adds much.

\section{Limitations and future work}
\label{sec:future}
\begin{itemize}[nosep,leftmargin=*]
  \item \textbf{Single model and simple graph tasks.} Our results come from one model
  ...
  \item \textbf{Single prompt.} We use one Proposer/Critic wording. The Critic's
    ...
    the debate fails on precision rather than on how well the Critic detects errors.
  \item \textbf{Debate structure.} We tested one minimal two-role debate. Richer
    structures - more agents or independent proposers that then interact with an LLM judge - may improve results.
\end{itemize}
...
measures what the critic detected as a wrong answer, not what answers were actually rescued.
