...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}llcccc@{}}
    \toprule
    \textbf{Task} & \textbf{Encoding} & \textbf{Detection} & \textbf{False alarm} &
      $\phi$ & \textbf{Net fixed $-$ broken} \\
    \midrule
    \texttt{connected\_nodes} & \texttt{incident}   & 0.327 & 0.102 & $+0.186$ & $-4$ \\
    \texttt{connected\_nodes} & \texttt{adjacency}  & 0.423 & 0.201 & $+0.217$ & $-11$ \\
    \texttt{connected\_nodes} & \texttt{friendship} & 0.308 & 0.085 & $+0.288$ & $+19$ \\
    \addlinespace
    \texttt{node\_degree} & \texttt{incident}   & 0.260 & 0.070 & $+0.185$ & $-4$ \\
    \texttt{node\_degree} & \texttt{adjacency}  & 0.298 & 0.102 & $+0.218$ & $-11$ \\
    \texttt{node\_degree} & \texttt{friendship} & 0.208 & 0.059 & $+0.208$ & $0$ \\
    \addlinespace
    \texttt{edge\_existence} & \texttt{incident}   & 0.857 & 0.105 & $+0.406$ & $-2$ \\
    \texttt{edge\_existence} & \texttt{adjacency}  & 0.957 & 0.290 & $+0.537$ & $-71$ \\
    \texttt{edge\_existence} & \texttt{friendship} & 0.926 & 0.161 & $+0.694$ & $-29$ \\
    \midrule
    \textbf{Pooled} & & 0.502 & 0.132 & $+0.358$ & $-113$ \\
    \bottomrule
  \end{tabular}
  \caption{Per-cell Critic diagnostic. Detection is $P(\textsc{revise}\mid\text{wrong})$,
  false alarm is $P(\textsc{revise}\mid\text{correct})$, $\phi$ is the association between
  a \textsc{revise} and a wrong answer, and net is answers fixed minus broken by revision.
  The loop is net-positive only on \texttt{connected\_nodes}/\texttt{friendship}.}
  ...
  \begin{tabular}{@{}llcccccc@{}}
    \toprule
    \textbf{Task} & \textbf{Encoding} & \textbf{Baseline} & \textbf{Turn 1} &
      \textbf{Debate} & \textbf{Vote} & $p$(deb$-$base) & $p$(vote$-$deb) \\
    \midrule
    \texttt{connected\_nodes} & \texttt{incident}   & 0.958 & 0.947 & 0.940 & 0.952 & 0.118 & 0.248 \\
    \texttt{connected\_nodes} & \texttt{adjacency}  & 0.743 & 0.835 & 0.817 & 0.822 & $<$0.001 & 0.804 \\
    \texttt{connected\_nodes} & \texttt{friendship} & 0.488 & 0.655 & 0.687 & 0.687 & $<$0.001 & 0.902 \\
    \addlinespace
    \texttt{node\_degree} & \texttt{incident}   & 0.873 & 0.942 & 0.935 & 0.943 & $<$0.001 & 0.302 \\
    \texttt{node\_degree} & \texttt{adjacency}  & 0.655 & 0.868 & 0.850 & 0.875 & $<$0.001 & 0.033 \\
    \texttt{node\_degree} & \texttt{friendship} & 0.637 & 0.828 & 0.828 & 0.837 & $<$0.001 & 0.551 \\
    \addlinespace
    \texttt{edge\_existence} & \texttt{incident}   & 0.980 & 0.978 & 0.975 & 0.985 & 0.678 & 0.109 \\
    \texttt{edge\_existence} & \texttt{adjacency}  & 0.938 & 0.922 & 0.803 & 0.950 & $<$0.001 & $<$0.001 \\
    \texttt{edge\_existence} & \texttt{friendship} & 0.867 & 0.800 & 0.752 & 0.822 & $<$0.001 & $<$0.001 \\
    \bottomrule
  \end{tabular}
  \caption{Per-cell accuracy under each condition, with paired McNemar $p$-values for
  debate vs.\ baseline and vote vs.\ debate. Turn~1 is debate's first-turn
  chain-of-thought (also each majority-vote draw's prompt).}
  \label{tab:percell-acc}
\end{table*}

Tables~\ref{tab:percell-critic} and~\ref{tab:percell-acc} report every cell's numbers
behind the pooled figures in \S\ref{sec:results}. $p$-values are paired McNemar tests
(cells share graphs); values below $0.001$ are shown as \texttt{<0.001}. ``Net fixed
$-$ broken'' is the number of answers a revision moved wrong$\to$right minus
right$\to$wrong, so a positive value means the loop helped that cell. Detection
